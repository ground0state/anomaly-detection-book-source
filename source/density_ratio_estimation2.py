...
class KLDensityRatioEstimation():
    ...
    def fit(self, X_normal, X_error):
        ...
        # sum of basic function with data
        xi = (self.psi_prime).mean(axis=0)
        xixi = xi@xi
        ...
        for _ in range(self.max_iter):
            ...
            temp += (1 - xi@temp)*xi/xixi
            temp = np.maximum(temp, 0)
            self.theta = temp/(temp@xi)

            # calculate density latio
            r = np.dot(self.psi, self.theta)

            # calculate loss
            immediate_loss = self._obj_func(r)
    ...
    def _obj_func(self, r):
        obj = - np.sum(np.log(r))
        return obj

    def _obj_deri_func(self, r):
        dobj = - (self.psi / r).sum(axis=0)
        return dobj
